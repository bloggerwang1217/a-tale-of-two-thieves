...
Location-specific variance components account for 31.9\% of total measurement variance (see Section 3.2.2 and Table \ref{tab:a3}), which is 13.87 times greater than the METHOD effect. This establishes unambiguously that V-Blender position is the dominant source of product variability.
...
\begin{table}[H]
	...
	\begin{tabular}{l rrc}
		...
		\midrule
		Between-Location [$\text{Var}(b_i)$] & 0.9976 & 31.9\% &
		...
		& 65.7\% & Weighted average residual \\
		\midrule
		METHOD Effect [$\text{Var}(\beta \cdot \text{METHOD})$] & 0.0731 &
		2.3\% & Fixed effect variance \\
		\midrule
		\textbf{Total Variance} & \textbf{3.1242} &
		\textbf{100.0\%} & \\
		\bottomrule
	\end{tabular}
	\\ \vspace{0.2cm}
	...
	measurement precision. The decomposition
	shows location effects (31.9\%) dominate over method
	choice (2.3\%).
\end{table}
