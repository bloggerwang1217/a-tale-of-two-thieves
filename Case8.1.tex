...
\subsubsection{Diagnostic Check}

\noindent\textbf{Normality Assessment:} Both the Intermediate Dose and Unit Dose thieves show normal distributions based on Shapiro-Wilk testing (see Table \ref{tab:a1a}). Intermediate Dose yields W-statistic = 0.9836 (p = 0.9794) and Unit Dose yields W-statistic = 0.9424 (p = 0.3190), both with p-values well above the conventional significance level of 0.05, confirming normal distributions. \textit{This finding addresses the client's Question 1 regarding whether assay values are generally well-behaved (see Section \ref{sec:q1} for detailed discussion).}

\noindent\textbf{Outlier Detection:} Conservative outlier detection using the criterion Q3 + 2×IQR identified zero outliers for both methods (see Table \ref{tab:a1b}). The Intermediate Dose upper outlier boundary is 41.47 mg/100mg and Unit Dose is 44.46 mg/100mg, both substantially above the observed maximum values, confirming absence of extreme values that could violate model assumptions.
...
\subsubsection{Thief-to-Tablet Comparison: Assessment of Measurement Bias}
